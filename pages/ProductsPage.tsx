...
import { Search, Plus, Package, Ruler, Pencil, Trash2, ChevronRight } from 'lucide-react';
import { mockWoodProducts } from '../lib/mock-data';
import { useNavigate } from 'react-router-dom';

const CATEGORIES = ['Alle', 'Deuren', 'Mouldings', 'Lijsten', 'Kozijnen', 'Crating'];
...
const ProductsPage: React.FC = () => {
  ...
  const filtered = mockWoodProducts.filter(p => {
    const matchSearch = p.name.toLowerCase().includes(search.toLowerCase()) ||
      p.woodType.toLowerCase().includes(search.toLowerCase());
    const matchCat = activeCategory === 'Alle' || (p as any).category === activeCategory;
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100 bg-slate-50/20 flex items-center gap-4">
          <div className="flex-1 max-w-sm relative">
            ...
            <input 
              aria-label="Zoeken in catalogus"
              type="text" 
              value={search}
              onChange={e => setSearch(e.target.value)}
              placeholder="Search wood species or product..." 
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 border-b border-slate-100 text-[10px] font-black text-slate-400 uppercase tracking-widest">
              <tr>
                <th className="px-6 py-4">Wood Species / Product</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((p) => (
                <tr key={p.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-right font-black text-slate-900 italic">{currencySymbol}{p.pricePerUnit}</td>
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <button 
                        title="Detail bekijken"
                        onClick={() => navigate(`/products/edit/${p.id}`)}
                        className="p-2 text-slate-400 hover:text-brand-primary hover:bg-red-50 rounded-lg transition-all"
                      >
