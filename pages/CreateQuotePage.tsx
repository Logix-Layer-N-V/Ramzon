...
const CreateQuotePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!isEdit && s) {
      if (s.clientId) setClient(s.clientId);
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm space-y-6">
          <div className="flex items-center gap-4 border-b border-slate-50 pb-4">
            <div className="w-12 h-12 bg-blue-50 text-blue-600 rounded-2xl flex items-center justify-center"><ClipboardList size={22} /></div>
            <div>
              <h2 className="text-xl font-black text-slate-900">{isEdit ? 'Edit Quote' : 'New Quote'}</h2>
              {!isEdit && <p className="text-xs font-black text-slate-400 font-mono">{docNumber}</p>}
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <input aria-label="Date" type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold outline-none" />
            </div>
            <div className="space-y-1.5">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
          ...
          {items.length > 0 && (
            <div className="border border-slate-200 rounded-2xl overflow-hidden">
              {/* Column header */}
              <div className="hidden md:grid md:grid-cols-[44px_76px_160px_38px_86px_84px_56px_72px_68px_22px] gap-1.5 px-3 py-1.5 bg-slate-50 border-b border-slate-200">
              ...
              {items.map((item, idx) => (
                ...
                <div
                  className="flex flex-wrap md:grid md:grid-cols-[44px_76px_160px_38px_86px_84px_56px_72px_68px_22px] gap-1.5 items-center px-3 py-2 border-b border-slate-100 hover:bg-slate-50/50 transition-colors group">
                  {/* Qty */}
          ...
          <div className="flex flex-wrap items-center gap-6 justify-between">
            <div className="flex flex-wrap gap-8">
              ...
              <div>
                ...
                <p className="text-3xl font-black text-brand-accent">{currencySymbol}{total.toFixed(2)}</p>
              </div>
            </div>
