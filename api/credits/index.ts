...
export async function GET(req: Request): Promise<Response> {
  ...
  try {
    const sql = getSql();
    const rows = await sql`SELECT * FROM credits ORDER BY date DESC`;
...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    ...
    const rows = await sql`
      INSERT INTO credits (client_id,amount,currency,date,reason)
      VALUES (${client_id},${amount},${currency},${date},${reason})
      RETURNING *`;
